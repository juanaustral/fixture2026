#!/usr/bin/env python3
"""Verify data.json matches Flashscore data"""
import json, re, sys

# Flashscore data from structured DOM extraction
flashscore_matches = """
27.06. 23:00 | Algeria vs Austria | 3-3
...
# Parse flashscore matches
fs_matches = {}
for line in flashscore_matches.strip().split('\n'):
    if '|' not in line:
        continue
    parts = [p.strip() for p in line.split('|')]
    if len(parts) >= 3:
        home = parts[1].split(' vs ')[0].strip()
        away = parts[1].split(' vs ')[1].strip() if ' vs ' in parts[1] else ''
        score = parts[2]
        if '-' in score:
            hs, as_ = score.split('-')
            key = (home.lower(), away.lower())
            fs_matches[key] = (hs, as_)

print(f"Parsed {len(fs_matches)} matches from Flashscore")

# English-to-Spanish team name map (data.json uses Spanish names)
team_name_map = {
    'méxico': 'mexico',
...
def normalize_name(name):
    """Normalize a team name from data.json to English for lookup"""
    n = name.lower().strip()
    n = re.sub(r'[^a-záéíóúñü ]', '', n).strip()
    return team_name_map.get(n, n)

def extract_opponent_name(opponent_field):
    """Extract just the team name from '🇿🇦 Sudáfrica' format"""
    parts = opponent_field.split()
    name_parts = []
    for p in parts:
        if all(ord(c) < 0x1F000 for c in p):
            name_parts.append(p)
...
for group in data['groups']:
    for team in group['teams']:
        ...
        for m in team.get('matches', []):
            checked += 1
            opp_name = extract_opponent_name(m['opponent'])
            opp_norm = normalize_name(opp_name)
            
            score = m['score'].replace('–', '-')
            if '-' not in score:
                errors.append(f"BAD SCORE FORMAT: {team['name']} vs {m['opponent']}: '{m['score']}'")
                continue
            team_score, opp_score = score.split('-')
            team_score = team_score.strip()
            opp_score = opp_score.strip()
            
            # Look up in both directions
            key_fwd = (team_norm, opp_norm)
            key_rev = (opp_norm, team_norm)
            
            if key_fwd in fs_matches:
                fhs, fas = fs_matches[key_fwd]
                if team_score != fhs or opp_score != fas:
                    errors.append(f"SCORE MISMATCH: {team['name']} vs {m['opponent']}: data={team_score}-{opp_score}, flashscore={fhs}-{fas}")
                else:
                    ok += 1
            elif key_rev in fs_matches:
                fhs, fas = fs_matches[key_rev]
                if team_score != fas or opp_score != fhs:
                    errors.append(f"SCORE MISMATCH (rev): {team['name']} vs {m['opponent']}: data={team_score}-{opp_score}, flashscore={fas}-{fhs}")
                else:
                    ok += 1
            else:
                # Try fuzzy matching
                found = False
                for (h, a), (hs, as_) in fs_matches.items():
                    if (team_norm in h and opp_norm in a):
                        if team_score == hs and opp_score == as_:
                            ok += 1
                        else:
                            errors.append(f"SCORE MISMATCH (fwd): {team['name']} vs {m['opponent']}: data={team_score}-{opp_score}, flashscore={hs}-{as_}")
                        found = True
                        break
                    elif (team_norm in a and opp_norm in h):
                        if team_score == as_ and opp_score == hs:
                            ok += 1
                        else:
                            errors.append(f"SCORE MISMATCH (rev): {team['name']} vs {m['opponent']}: data={team_score}-{opp_score}, flashscore={as_}-{hs}")
                        found = True
                        break
                if not found:
                    errors.append(f"NOT FOUND: {team['name']} vs {m['opponent']} (norm: '{team_norm}' vs '{opp_norm}')")
