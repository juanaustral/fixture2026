#!/usr/bin/env python3
"""Verify data.json matches Flashscore - using flag emoji mapping"""
import json, sys

# Build flag -> English name mapping from data.json
# Each team has a flag in t['flag'] and name in t['name']
# Each match has opponent which is a flag emoji
...
# Spanish -> English name map (for team names, not flag-based)
spanish_to_en = {
    'méxico': 'mexico',
...
# Build flag-to-Spanish-name mapping from data.json itself
flag_to_spanish = {}
for g in data['groups']:
    for t in g['teams']:
        flag_to_spanish[t['flag']] = t['name']
...
for group in data['groups']:
    for team in group['teams']:
        ...
        for m in team.get('matches', []):
            total += 1
            opp_flag = m['opponent']
            opp_es_name = flag_to_spanish.get(opp_flag, 'UNKNOWN')
            opp_es_lower = opp_es_name.lower().strip()
            opp_en = spanish_to_en.get(opp_es_lower, '???')
            
            score_data = m['score'].replace('–', '-')
            if '-' not in score_data:
                errors.append(f"BAD SCORE: {team['name']} vs flag {opp_flag}: '{m['score']}'")
                continue
            ...
            if fwd_key in fs_map:
                ...
                if data_ts == fhs and data_os_ == fas:
                    ...
                    errors.append(f"MISMATCH: {team['name']} vs {opp_es_name} (en:{team_en}-{opp_en}): data={data_ts}-{data_os_}, FS={fhs}-{fas}")
                    matched = True
                ...
                if data_ts == r_as and data_os_ == r_hs:
                    ...
                    errors.append(f"MISMATCH(rev): {team['name']} vs {opp_es_name} (en:{team_en}-{opp_en}): data={data_ts}-{data_os_}, FS={r_as}-{r_hs}")
                    matched = True
            
            if not matched:
                errors.append(f"NOT FOUND: {team['name']} vs {opp_es_name} (en:{team_en}-{opp_en})")
